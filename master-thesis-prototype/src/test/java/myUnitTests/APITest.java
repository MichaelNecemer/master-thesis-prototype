...
public class APITest {
	...
	public void testAllLanesAsParticipantsInGlobalSphere() {
		...
		try {
			...
			Assert.assertTrue(expected.size() == globalSphereList.size() && 
				expected.containsAll(globalSphereList) && globalSphereList.containsAll(expected));
	...
	public void testStaticSpheresForDataObjects() {
		...
		try {
			...
			Assert.assertTrue(expectedStaticSphereForD1.size() == staticSphereList.size() && 
					expectedStaticSphereForD1.containsAll(staticSphereList) && staticSphereList.containsAll(expectedStaticSphereForD1));
...
ArrayList<Double> cost = new ArrayList<>(Arrays.asList(10.0, 5.0, 3.0, 2.0));
		...
		try {
			...
			HashMap<Boolean, LinkedList<LinkedList<BPMNElement>>>list = a2.allEffectivePathsForWriters(dataO, writerTask, writerTask, a2.getBpmnEnd(), new LinkedList<BPMNElement>(), new LinkedList<BPMNElement>(), new LinkedList<BPMNElement>(), new LinkedList<LinkedList<BPMNElement>>());
...
ArrayList<Double> cost = new ArrayList<>(Arrays.asList(10.0, 5.0, 3.0, 2.0));
		...
		try {
			...
			HashMap<Boolean, LinkedList<LinkedList<BPMNElement>>>effectivePaths = a2.allEffectivePathsForWriters(dataO, writerTask, writerTask, a2.getBpmnEnd(), new LinkedList<BPMNElement>(), new LinkedList<BPMNElement>(), new LinkedList<BPMNElement>(), new LinkedList<LinkedList<BPMNElement>>());
			
			
			String sphereForDI = a2.getSphereForParticipantOnEffectivePathsWithAlreadyChosenVoters(writerTask, dataO, readerDI, effectivePaths, new HashMap<BPMNBusinessRuleTask, LinkedList<BPMNParticipant>>());
			Assert.assertEquals("Static", sphereForDI );
			String sphereForGP = a2.getSphereForParticipantOnEffectivePathsWithAlreadyChosenVoters(writerTask, dataO, readerGP, effectivePaths, new HashMap<BPMNBusinessRuleTask, LinkedList<BPMNParticipant>>());
			Assert.assertEquals("Static", sphereForGP );			
			String sphereForRS = a2.getSphereForParticipantOnEffectivePathsWithAlreadyChosenVoters(writerTask, dataO, readerRS, effectivePaths, new HashMap<BPMNBusinessRuleTask, LinkedList<BPMNParticipant>>());
			Assert.assertEquals("Strong-Dynamic", sphereForRS );
			String sphereForIC = a2.getSphereForParticipantOnEffectivePathsWithAlreadyChosenVoters(writerTask, dataO, readerIC, effectivePaths, new HashMap<BPMNBusinessRuleTask, LinkedList<BPMNParticipant>>());
	...
	@Test
	public void testSphereOfReaderParticipantsForWriter1ForBrt3() {
ArrayList<Double> cost = new ArrayList<>(Arrays.asList(10.0, 5.0, 3.0, 2.0));
